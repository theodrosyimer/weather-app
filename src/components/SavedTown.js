  ...
  return `
      <div data-lat="${location.lat}" data-lon="${location.lon}"  class="city-card-grid">
          <div class="flex">
            ...
            <div class="current-temperature">${Math.round(current.temp_c)}°</div>
          </div>

          <div class="flex">
            <div class="short-description">${current.condition.text}</div>
            <div class="highest-lowest-temperatures">
              H: <span class="highest-temp">${Math.round(forecastday[0].day.maxtemp_c)}°</span>
              L: <span class="lowest-temp">${Math.round(forecastday[0].day.mintemp_c)}°</span>
            </div>
          </div>
        </div>
  `
